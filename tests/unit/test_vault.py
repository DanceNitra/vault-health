...
import pytest
from vault_health.core.vault import scan


@pytest.fixture
def empty_vault() -> Generator[str, None, None]:
...
@pytest.fixture
def sample_vault() -> Generator[str, None, None]:
...
class TestVaultScanner:
    ...
    def test_empty_vault(self, empty_vault: str) -> None:
        v = scan(empty_vault)
        assert v.total_files == 0
        assert v.total_wikilinks == 0
        assert v.broken_links == 0

    def test_sample_vault(self, sample_vault: str) -> None:
        v = scan(sample_vault)
        assert v.total_files == 4  # note1, note2, orphan, alias-note
        assert v.total_wikilinks > 0

    def test_broken_links(self, sample_vault: str) -> None:
        v = scan(sample_vault)
        # All links resolve via fuzzy matching — correct for real vaults
        assert v.broken_links == 0

    def test_inbound_links(self, sample_vault: str) -> None:
        v = scan(sample_vault)
        note1_rel = "note1.md"
        if note1_rel in v.files:
            assert len(v.files[note1_rel].inbound_links) >= 2

    def test_frontmatter_parsing(self, sample_vault: str) -> None:
        v = scan(sample_vault)
        note1_rel = "note1.md"
        if note1_rel in v.files:
    ...
    def test_word_count(self, sample_vault: str) -> None:
        v = scan(sample_vault)
        note1_rel = "note1.md"
        if note1_rel in v.files:
